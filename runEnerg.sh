...
echo $max
min=`sort -nk 3 $infile | head -n 1 | awk '{print $3}'`
echo $min
fix=0.0001
set=$(echo "$min"-"$fix" | bc)
echo $set
...
####### Resample it ################################################################
grdsample topo0.grd -Gtopoh.grd -I01m -Q0.0001 -Lxy

####### Convert the xyz file into GRD file #########################################
awk '{print $1, $2, $3}' $infile | xyz2grd -R -D:degree:degree -N$set -I0.6/0.6 -Gtmp.grd
#awk '{print $1, $2, $3}' $infile | xyz2grd -R -D:degree:degree -N-0.0001 -An -I0.5/0.5 -Gtmp.grd

# -N0 setting empty nodes
# -A options: l_lowest(minimum) u_upper(maximun) n_count number of data points per node

####### Resample tmp.grd from coarse (1.0 deg.) into finer resolution (0.5 degree) using grdsample.
grdsample tmp.grd -Gtmph.grd -I1m -Q0.0001 -Lxy

####### Make a color table ##########################################################
makecpt -Cseis -T$set/$max/0.5 -I >tmp.cpt
#makecpt -Cseis -T1/50/1 -I >tmp.cpt
...
# more dramatic shade.
#####################################################################################
grdgradient topoh.grd -Gtopoh.int -A45 -Ne1
...
psscale -D2.5/-1.0/6.0/0.30h -B1/:Log_M0: -Ctmp.cpt -I -O >> $out
